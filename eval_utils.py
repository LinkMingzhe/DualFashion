...
from eval.compatibility_evaluator.compatibility_net import FashionEvaluator

class InceptionV3(nn.Module):
    def __init__(self, model_path, num_classes, map_location=None):
...
def calculate_inception_score_given_data(data4eval, cate4eval, model_path, num_classes,
    ...
    model = InceptionV3(model_path, num_classes, map_location=map_location).to(device)
    ...
    for batch, cate_batch in zip(dataloader, cate_dataloader):
        ...
        topk_labels = torch.topk(pred, k=3, dim=1).indices
        correct = torch.sum((topk_labels == cate_batch.unsqueeze(1)).any(dim=1)).item()
...
@torch.no_grad()
def calculate_pairwise_lpips_given_groups(groups, device, use_net="vgg"):
    model = lpips.LPIPS(net=use_net).to(device)
    total_score = 0.0
    total_pairs = 0
    ...
    for tensors in groups:
        if tensors is None or len(tensors) < 2:
            continue
        stack = torch.stack([tensor.float() for tensor in tensors]).to(device)
        num_imgs = stack.size(0)
        for i in range(num_imgs):
            img_i = stack[i : i + 1]
            for j in range(i + 1, num_imgs):
                img_j = stack[j : j + 1]
                score = model(img_i, img_j)
