...
class ConditionalMaskedDiffusionSampler:
    ...
    # the model needs to be wrapped with SUBS parameterization at first
        ...
        self.conditioning_embeds = None

    def register_condition(self, y, x, x_mask, conditioning_embeds=None):
        device = y.device
        self.y = y
        self.x = x.to(device)
        self.x_mask = x_mask.to(device)
        self.conditioning_embeds = conditioning_embeds
    ...
    def forward(self, xt):
        ...
        seq_len = xt.shape[1]
        if self.conditioning_embeds is not None:
            cond_embeds = self.conditioning_embeds.to(
